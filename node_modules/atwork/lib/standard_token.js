...
const getAccessTokenKey = function() { //支持多个应用同时使用同一个atwork实例
	return `${this.domainId}_${this.orgId}_${this.clientId}_${this.clientSecret}`;
};
const send = function(options) {
	return new Promise((success, fail) => {
		console.log("申请token参数是\n", options);
		request(options, (e, res, body) => {
			console.log("申请token响应是", body);
			if (e) {
				console.error("网络错误");
				fail(e);
			} else if (body.status !== 0) {
				console.error("token服务器返回非0，申请失败");
...
const applyForToken = function() {
	...
	return send(options)
		...
		.then(token => token.result.access_token)
		.catch(e => {
...
const resetToken = function() { //强制刷新token
	...
	if (tokenObj.status === READY) {
		console.warn("等待完毕，使用别人申请到的token", tokenObj.token.result.access_token);
		return Promise.resolve(tokenObj.token.result.access_token);
	}
...
const getAccessToken = function() { //异步申请token，如果有缓存的话会优先使用缓存
	...
	if (token && token.status === READY) {
		...
		return Promise.resolve(token.token.result.access_token);
	} else {
...
const isTokenExist = function() {
	throw new Error("should not invoke this method in standard mode");
};
